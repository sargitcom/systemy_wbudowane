...
# 57 "main.c" 2
# 79 "main.c"
unsigned int isAlarmOn = 0;
unsigned int isAlarmTurnedOn = 1;
...
void updateStateOfTheReset()
{
    ...
    if(PORTBbits.RB3 == 0)
    {
        isAlarmTurnedOn = isAlarmTurnedOn == 1 ? 0 : 1;

        if (isAlarmTurnedOn == 0) {
            PORTD = 0;
            isAlarmOn = 0;
        }
    }
...
void main(void) {
    ...
    while(1) {
        ...
        if (isAlarmTurnedOn == 1) {
            unsigned int trap = adc(0);

            if (trap > 375) {

                if (isAlarmOn == 0) lightDiod();
